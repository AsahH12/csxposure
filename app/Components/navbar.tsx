      ...
      <ul className="navbar-end d-flex align-items-center">
        ...
        <div className="nav-item dropdown mx-2">
          <a className="nav-link dropdown-toggle" data-bs-toggle="dropdown">
            {profileImage ? (
              <img src={profileImage} alt="Profile" width={50} height={50} className="rounded-circle border" />
            ) : (
              <div className="rounded-circle border d-flex justify-content-center align-items-center" style={{ width: 50, height: 50, backgroundColor: "#ccc" }}>
                {initials}
              </div>
            )}
          </a>
          <div className="dropdown-menu">
            {!user && <Link href="/Authentication" className="dropdown-item">Login/SignUp</Link>}
            {user && userType && (
              <>
                <Link href={userType === "business" ? "/BusinessEditPage" : "/ProfileEditPage"} className="dropdown-item">My Profile</Link>
